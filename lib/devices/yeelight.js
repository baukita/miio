...
module.exports.ColorTemperature = Thing.mixin(
	(Parent) =>
		class extends Parent.with(MiioApi, Colorable, ColorTemperature) {
			constructor(...args) {
				super(...args);
				...
				if (color.is('temperature')) {
					...
					this.colorTemperatureRange().then((range) => {
						...
						return this.call(
							...
							Yeelight.withEffect(temperature, options.duration),
							{
								refresh: ['colorTemperature', 'colorMode', 'nl_br'],
							}
						).then(MiioApi.checkOk);
					});
					...
					return this.call(
						...
						{
							refresh: ['colorRGB', 'colorMode', 'nl_br'],
						}
					).then(MiioApi.checkOk);
				}
			}
