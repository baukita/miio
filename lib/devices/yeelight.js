...
const NightMode = require('./capabilities/nightMode');
	...
	(Parent) =>
		class Yeelight extends Parent.with(
			...
			constructor(options) {
				...
				// Query for the color mode
				this.defineProperty('active_mode', {
					name: 'activeMode',
					mapper: (v) => {
						v = parseInt(v);
						switch (v) {
			...
			changePower(power, mode = 1) {
				// TODO: Support for duration
				return this.call(
					'set_power',
					[...Yeelight.withEffect(power ? 'on' : 'off'), mode],
					{
						refresh: ['power', 'activeMode', 'brightness', 'nlBrightness'],
					}
					...
					return promise
						.then(() =>
							this.call(
								...
								{
									refresh: ['brightness', 'nlBrightness'],
								}
							)
						)
						.then(MiioApi.checkOk);
			 ...
			 */
			static withEffect(arg, duration, mode) {
...
module.exports.ColorTemperature = Thing.mixin(
	(Parent) =>
		class extends Parent.with(MiioApi, Colorable, ColorTemperature, NightMode) {
			constructor(...args) {
				super(...args);
			...
			changeColor(color, options) {
				return this.colorTemperatureRange().then((range) => {
					...
					return this.call(
						'set_ct_abx',
						Yeelight.withEffect(temperature, options.duration),
						{
							refresh: ['colorTemperature', 'activeMode', 'nlBrightness'],
						}
					).then(MiioApi.checkOk);
				});
			}
...
module.exports.ColorFull = Thing.mixin(
	(Parent) =>
				...
				if (
					...
					switch (this.property('colorMode')) {
						case 'colorTemperature':
							...
							currentColor = color.temperature(
								this.property('colorTemperature')
							);
							break;
						case 'rgb': {
				...
				if (color.is('temperature')) {
					...
					this.colorTemperatureRange().then((range) => {
						...
						return this.call(
							...
							Yeelight.withEffect(temperature, options.duration),
							{
								refresh: ['colorTemperature', 'colorMode', 'nlBrightness'],
							}
						).then(MiioApi.checkOk);
					});
					...
					return this.call(
						'set_rgb',
						Yeelight.withEffect(rgb, options.duration),
						{
							refresh: ['colorRGB', 'colorMode', 'nlBrightness'],
						}
					).then(MiioApi.checkOk);
				}
			}
